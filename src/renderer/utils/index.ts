 ...
 * @returns {Box3} 包围盒
 */
export function getBoundingBoxInWorldSpace(box: Box3D): Box3 {
  ...
  // 本地空间 ---> 世界空间
  minProject.applyMatrix4(box.matrixWorld);
  maxProject.applyMatrix4(box.matrixWorld);
 ...
 */
export function getBoundingBoxInCameraSpace(box: Box3D, camera: Camera): Box3 {
  if (!box.geometry.boundingBox) box.geometry.computeBoundingBox();
  const bbox = box.geometry.boundingBox!;
  const minProject = bbox.min.clone();
  const maxProject = bbox.max.clone();
